...
/*   Updated: 2023/07/17 15:39:05 by kchan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	read_and_stash(int fd, char **buffer, char **stash)
{
	...
	while (!ft_strchr(*stash, '\n') && nbyte_read > 0)
	{
		...
		(*buffer)[nbyte_read] = '\0';
		temp_stash = *stash;
		join_str = ft_strjoin(temp_stash, *buffer);
		*stash = join_str;
		free_ptr(&temp_stash);
	}
	return (nbyte_read);
...
char	*getnewline(int fd, char **buffer, char **stash)
{
...
char	*get_next_line(int fd)
{
	...
	if (!stash[fd])
		stash[fd] = ft_strdup("");
	line = getnewline(fd, &buffer, &stash[fd]);
	free_ptr(&buffer);
	return (line);
}
